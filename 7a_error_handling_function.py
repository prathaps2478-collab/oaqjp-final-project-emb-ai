...
Task 7: Updated emotion_detector function with error handling for status code 400.
"""
import requests
import json
...
def emotion_detector(text_to_analyse):
    ...
    Handles blank input by returning None values when status code is 400.

    Args:
        text_to_analyse (str): The text to analyze for emotions.

    Returns:
        dict: A dictionary with emotion scores and dominant_emotion.
              Returns None values for all keys if input is blank (status 400).
    """
    url = (
    ...
    # Handle blank or invalid input — Watson returns status 400
